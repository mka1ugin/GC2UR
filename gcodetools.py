...
def process_gcode_string(gcommand):
    ...
    if gcommand[0] == "G0" or gcommand[0] == "G1":  # если это команда линейного перемещения
        ...
        x0, y0, z0, x1, y1, z1, x2, y2, z2 = getCircularMove(gcommand)
...
def spindleOn():
    global analogOutValue

    if analogAffector:
        analogOutValue = "%.2f" % (analogOutValue * 0.1)
        return f"set_analog_out({analogOutNumber}, {analogOutValue})"
    else:
        return f"set_digital_out({digitalOutNumber}, True)"

def spindleOff():
    if analogAffector:
        return f"set_analog_out({analogOutNumber}, 0)"
    else:
        return f"set_digital_out({digitalOutNumber}, False)"
...
def getCircularMove(gcommand):
    ...
    x1, y1 = arcCenter(x0, y0, x2, y2, x_center, y_center, direction, quad)
